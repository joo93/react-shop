...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    setCartItemCount(Object.keys(cart.items || {}).length);
  }, [cart]);

  const toggleTheme = useCallback(() => {
    setTheme((prevTheme) => {
      const newTheme = prevTheme === CONSTANTS.THEME.LIGHT ? CONSTANTS.THEME.DARK : CONSTANTS.THEME.LIGHT;
  ...
  return (
    <div className="fixed z-10 w-full navbar shadow-lg bg-white dark:bg-neutral-900 text-neutral-content">
      <div className="flex w-full xl:container xl:m-auto">
        <label
          htmlFor="side-menu"
          className="flex-none lg:hidden btn btn-square btn-ghost w-10 sm:w-auto dark:hover:bg-zinc-700"
        >
          <svg
        ...
        <div className="flex-none hidden md:flex md:flex-1 ml-2">
          {menus.map((menu) => (
            <Link
              to={`/${menu.name}`}
              key={menu.name}
              className="btn btn-ghost btn-sm rounded-btn text-gray-700 dark:text-white dark:hover:bg-zinc-700"
            >
              {menu.title}
            </Link>
          ))}
        ...
        <div className="flex items-center px-2 relative">
          <label className="swap swap-rotate mr-2 sm:mr-4" htmlFor="theme-toggle">
            <input
              type="checkbox"
              className="js-theme"
              id="theme-toggle"
            ...
            <svg className="swap-off fill-black w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <Link to="/cart" className="btn btn-ghost sm:w-12 pl-2 pr-2 dark:hover:bg-zinc-700">

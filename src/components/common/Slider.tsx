  ...
  return (
    ...
    >
      {items.map((item: ISliderItem, index: number) => {
        return (
          <div key={item.name} className="carousel-slide max-h-52 lg:max-h-[700px] xl:max-h-[700px] ">
            <div className="carousel-description absolute left-auto top-1/4 lg:bottom-1/3 right-auto mb-10 text-left w-full lg:container px-4 md:px-10 ">
            ...
            <img src={item.img} alt={item.name} className="object-cover object-center h-full" />
          </div>
        );
      })}
    </Carousel>
